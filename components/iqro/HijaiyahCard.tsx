...
interface HijaiyahCardProps {
    ...
    isLoading: boolean;
    isPlaying: boolean;
    // FIX: Updated onPlay to accept both char and latin for sequential speaking
    onPlay: (char: string, latin: string) => void; 
    isLarge?: boolean;
}
...
export const HijaiyahCard: React.FC<HijaiyahCardProps> = ({
    ...
    isLoading,
    isPlaying,
    onPlay, // Changed to accept char and latin
    isLarge = false
}) => {
    ...
    return (
        <div className="relative group">
            <button
                // FIX: Pass both item.char and item.latin to onPlay
                onClick={() => onPlay(item.char, item.latin)}
                disabled={isLoading}
            ...
            >
             ...
             <button 
                ...
                className="absolute bottom-1 right-1 p-1 text-gray-400 hover:text-gold-dark transition opacity-0 group-hover:opacity-100 focus:opacity-100 min-w-[44px] min-h-[44px]"
