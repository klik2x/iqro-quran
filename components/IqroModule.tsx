  ...
  return (
      <div className="bg-white dark:bg-dark-blue-card p-4 sm:p-6 rounded-2xl shadow-md">
        ...
        <div className="flex justify-center flex-wrap gap-2 mb-6">
            <button onClick={() => setViewMode('study')} className={`px-4 py-2 text-sm font-semibold rounded-lg flex items-center gap-2 ${viewMode === 'study' ? 'bg-emerald-dark text-white' : 'bg-gray-100 dark:bg-dark-blue'}`}><BookOpen size={16}/> {t('study')}</button>
            <button onClick={() => setViewMode('practice')} className={`px-4 py-2 text-sm font-semibold rounded-lg flex items-center gap-2 ${viewMode === 'practice' ? 'bg-emerald-dark text-white' : 'bg-gray-100 dark:bg-dark-blue'}`}><Play size={16}/> {t('practice')}</button>
            <button onClick={() => setViewMode('tajwid')} className={`px-4 py-2 text-sm font-semibold rounded-lg flex items-center gap-2 ${viewMode === 'tajwid' ? 'bg-emerald-dark text-white' : 'bg-gray-100 dark:bg-dark-blue'}`}><Award size={16}/> {t('tajwid')}</button>
            <button onClick={() => setViewMode('quiz')} className={`px-4 py-2 text-sm font-semibold rounded-lg flex items-center gap-2 ${viewMode === 'quiz' ? 'bg-emerald-dark text-white' : 'bg-gray-100 dark:bg-dark-blue'}`}><HelpCircle size={16}/> {t('quiz')}</button>
            <button onClick={() => setViewMode('bookmarks')} className={`px-4 py-2 text-sm font-semibold rounded-lg flex items-center gap-2 relative ${viewMode === 'bookmarks' ? 'bg-emerald-dark text-white' : 'bg-gray-100 dark:bg-dark-blue'}`}>
              <Bookmark size={16}/> {t('bookmarksMode')}
              {bookmarks.length > 0 && <span className="absolute -top-1 -right-1 w-4 h-4 text-xs bg-red-500 text-white rounded-full">{bookmarks.length}</span>}
            </button>
        </div>
