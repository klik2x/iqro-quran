...
const AudioPlayer: React.FC = () => { // Removed t prop, will use useTranslation hook directly
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center h-96 gap-6">
      ...
      <p className="text-slate-500 font-black text-xl animate-pulse">{t('preparingAudio' as TranslationKeys)}...</p>
    </div>
  );
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in duration-700 px-4">
      <div className="text-center md:text-left">
        <h1 className="text-5xl font-black mb-3 tracking-tighter text-slate-950 dark:text-white">{t('murotalPlayer' as TranslationKeys)}</h1>
        <p className="text-slate-500 text-lg font-medium">{t('murotalPlayerIntro' as TranslationKeys)}</p>
      </div>
            ...
            <div className="pt-8 flex flex-col items-center gap-8">
               ...
               {isPlaying && (
                <div className="flex flex-col items-center gap-4 animate-in fade-in duration-1000">
                  ...
                  <p className="text-emerald-700 dark:text-emerald-400 text-sm font-black uppercase tracking-widest">{t('playingAudio' as TranslationKeys)}...</p>
                  <audio 
                    autoPlay 
                    src={audioUrl} 
      ...
      <div className="bg-emerald-50/50 dark:bg-emerald-900/10 rounded-[3rem] p-10 border-2 border-emerald-100 dark:border-emerald-800 shadow-xl">
         <h3 className="text-2xl font-black flex items-center gap-3 text-emerald-900 dark:text-emerald-400 mb-8 tracking-tight">
            <ListMusic size={28} />
            {t('popularPlaylist' as TranslationKeys)}
         </h3>
               ...
               >
                 <div className="flex flex-col items-start">
                    ...
                    <span className="text-lg font-black text-slate-950 dark:text-white group-hover:text-emerald-600 transition-colors">
                      {surahs.find(s => s.number === num)?.englishName || `${t('surah' as TranslationKeys)} ${num}`}
                    </span>
                 </div>
