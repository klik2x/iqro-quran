...
const JuzDetail: React.FC = () => {
  ...
  const load = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const res = await fetchJuz(parseInt(number), translationLang); 
      setData(res);
  ...
  const handleWhatsAppShare = (ayah: Ayah, translation: string) => {
    const surahName = ayah.surah?.englishName || t('surah' as TranslationKeys);
    const text = `*${ayah.text}*\n\n${translation}\n\n(QS. ${surahName}: ${ayah.numberInSurah})`;
  ...
  const juzNum = parseInt(number || '1');
        ...
        {arabicEd.ayahs.map((ayah: Ayah, index: number) => { 
          ...
          return (
            <React.Fragment key={ayah.number}>
                {showSurahHeader && (
                    <div className="text-center py-6 my-8 border-y-2 border-dashed border-emerald-200 dark:border-emerald-800/50 bg-emerald-50/50 dark:bg-emerald-900/10 rounded-2xl">
                        <h3 className="text-xl font-bold text-emerald-dark dark:text-emerald-light">{ayah.surah.englishName}</h3>
                        <p className="font-arabic text-3xl text-emerald-dark dark:text-emerald-light">{ayah.surah.name}</p>
                    </div>
                )}
                {ayah.numberInSurah === 1 && ayah.surah.number !== 1 && ayah.surah.number !== 9 && (
                    <div className="text-center pb-8 mb-8 border-b border-emerald-100 dark:border-emerald-900/30">
                        <p className="font-arabic text-2xl text-slate-900 dark:text-white">بِسْمِ ٱللّٰهِ الرَّحْمٰنِ الرَّحِيْمِ</p>
                    </div>
                 )}
